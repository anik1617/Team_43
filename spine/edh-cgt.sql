...
('N22','decision',      'transfer_feasible_within_window',                                             1, NULL,                 'NICE NG232 1.4.16, 1.8.1 (discuss/transfer regardless of imaging) [VERIFY-NICE]; Peshawar transfer+tele-consult bridge. GRADUATED v4 (mentor-signed VF-GRAD-2): branch on transfer feasibility so a no-transfer setting still gets grounded maximal medical, never a dead end', 1),
('L22','leaf',          NULL,                                                                          0, 'STABILIZE_TRANSFER', 'NICE 1.4.16/1.8.1 [VERIFY-NICE]; Peshawar (transfer + tele-consult). Severe/deterioration/focal/penetrating/anticoag WITHOUT complete herniation cluster, transfer FEASIBLE -> abstain local op, stabilize, transfer', 1),
('L22m','leaf',         NULL,                                                                          0, 'STABILIZE_TRANSFER', 'GRADUATED v4 (VF-GRAD-2): transfer NOT feasible -> grounded maximal medical + keep attempting transfer/tele-consult; non-operative (no herniation cluster)', 1),
...
('N98','terminal',      'age_yr',                                                                      1, 'STABILIZE_TRANSFER', 'GRADUATED v4 (mentor-signed VF-GRAD-1): age<15 - adult BTF SBP bands + adult GCS do NOT apply; give LABELED peds stabilization (not a bare abstain) + transfer; operative step still N40 ABSTAIN; peds SBP ~70+2*age = labeled principle', 2),
('L98','leaf',          NULL,                                                                          0, 'STABILIZE_TRANSFER', 'GRADUATED v4 (VF-GRAD-1): grounded peds stabilization + transfer + tele-consult; no adult thresholds applied', 2),
...
('L21b','N40','true'),
('L21c','N40','true'),
-- N22 / N23 routes (GRADUATED v4 VF-GRAD-2: N22 branches on transfer feasibility; L22m = no-transfer maximal medical)
...
('N00', 4, 'UNSIGNED-pending-ed25519-at-bundle-compile');

-- ============================================================
-- GRADUATED-ASSISTANCE CHANGES — APPLIED in v4 (MENTOR-SIGNED VF-GRAD-1/2/3, 2026-06-20)
-- ------------------------------------------------------------
-- Status: APPLIED LIVE in the active tree above (cgt_meta.version = 4). The blocks
--   below are kept as a CHANGELOG of exactly what changed from v3 -> v4 (record
--   only, commented out). Re-sign the bundle (ed25519) at compile time.
