...
> **Handoff note (2026-06-20):** **Gowrish now owns this whole repo.** (Previously: Aniket = cloud, Gowrish = edge.) Work splits into two streams: **Aniket = cloud / knowledge pipeline → signed bundle** (no heavy compute — the ~160-node graph builds on a cloud LLM API); **Gowrish = the edge app + on-device engine + all heavy compute** — he alone has the **supercomputer + MIMIC/PhysioNet**, so the evals, the MIMIC arm, and any fine-tune run on his side. Full split: `docs/08-build-plan-and-task-split.md` §2. This file and the `docs/` set were consolidated so a fresh agent inherits full, coherent, contradiction-free context with no drift.
...
- **L2 — Knowledge.** A small, curated, source-cited **GraphRAG** graph (a ~160-node-class graph is proven sufficient), built on the **cloud build plane** (a cloud LLM API + light compute — a ~160-node corpus needs **no supercomputer**), shipped as a **signed SQLite bundle** (`edh-core-v{N}.kyro`). Uses MedGraphRAG's Triple-Graph idea (entity/source/definition) for citations and MedRAG discriminability (1/degree-centrality) to order questions with **no LLM call**.
- **L3 — Language I/O only (a mouth, not a brain).** A **stock Qwen2.5-3B-Q4** model via llama.rn + whisper.cpp (ASR) + Piper/OS-TTS. **Multilingual is IN v1:** UI + spine prompts + the synthesized recommendation ship in **English, Urdu, and Hindi** (other major languages as content allows). This is cheap because the schema is **i18n-native** (`cgt_strings.lang`): the **knowledge stays English** (sources are English guidelines) and we **translate only at the edges** — authored spine strings + the model phrasing the final recommendation *in the target language* — so we never re-embed the corpus per language. **Voice:** English ASR is primary; **Urdu/Hindi ASR via Whisper is best-effort** (small-model quality in a noisy emergency is the genuine risk — multilingual *text* is the reliable floor, voice is the stretch). The model does **four narrow jobs only**: clean ASR text, classify the operator's answer at each node, phrase the next question (in-language), and assemble the final cited recommendation **from the leaf the deterministic tree reached**. **Nothing load-bearing sits on the critical path.**
...
- **Scope (v1):** acute-TBI emergency pathway, deepest on **EDH**, **multilingual** (English + Urdu + Hindi text/UI/recommendations; English voice primary, Urdu/Hindi voice best-effort). **In:** L1 CGT spine + L2 small GraphRAG bundle + L3 **multilingual** voice I/O + graduated-assistance gating (🟢/🟡/🔴) + the state machine + **real WhatsApp escalation** (Twilio — the one *online* moment: Kyro is offline for the **decision**, and reaches out to hand off the pre-briefed state when a signal exists, degrading to on-screen / SMS when it doesn't) + a ~30-vignette benchmark + the spine-ablation chart. **Roadmap (cut from v1):** fine-tuning (use **stock Qwen2.5-3B first**; fine-tune is a later swap), multi-agent, broader neuro/TBI coverage beyond EDH, the full community-contribution flywheel/portal.
...
> **First artifact to build:** the acute-TBI/EDH **Clinical Guidance Tree** (the L1 spine), mentor-signed — it is both the credibility core and the benchmark answer key. Then the **E0 spike**: measure Qwen2.5-3B-Q4 tokens/sec + peak RAM on the real phone, and confirm op-sqlite loads sqlite-vec on Android.
