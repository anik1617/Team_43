...
**Kyro** (named for Chiron, who taught the healer Asclepius — *the knowledge in the room when the master can't be*) is an **offline, "expert-in-the-loop" surgical co-pilot** that delivers a neurosurgeon's validated procedure to a non-neurosurgeon in real time, **narrowed to epidural-hematoma (EDH) burr-hole evacuation** (the case's exact procedure). Full product context: [07-kyro-product-and-pitch.md](07-kyro-product-and-pitch.md). It replaces the failure point in the case: the GMO's teleconsult to a distant neurosurgeon that **connected briefly, then dropped — twice.** Built as a **GraphRAG** stack for the MVP (simpler, and its source-provenance is a credibility upgrade with skeptical MD judges); **Exo's cognitive substrate** is the *post-hackathon* architecture (TRHN for temporal procedure tracking, the DDM under the confidence gate, identity manifold for state continuity across dropped connections). An offline, on-device quantized model synthesizes answers from a local, **source-cited** knowledge graph that ships pre-loaded with canonical neurosurgery/surgery/triage knowledge and is grown by a **curated network of verified expert contributors** (the moat). Full design in [05-architecture.md](05-architecture.md). **The sharpened thesis: the problem isn't knowledge — it's *continuity*.** The case GMO had guidance and succeeded; what failed was the lifeline breaking mid-procedure (the call dropped, twice). Kyro's core innovation is a **procedure state machine** that captures operative state passively from voice, so a dropped call loses nothing and any reconnection auto-generates a pre-briefed handoff.
...
| 3. Business model / market | Underdeveloped — who pays? competitors? | **Kyro = Exo's philanthropic wing**; NGO licensing (MSF, PIH) + ministry procurement + Gates/Wellcome/Fogarty grants; beachhead **Namibia**; Mission:Brain's 130 chapters as contributor seed; competitors (SurgiAI, AI Virtual Operative Assistant, Indonesia tele-proctoring) all assume connectivity — see [07](07-kyro-product-and-pitch.md) |
| 4. Validation / MVP | Weak — "train on podcasts" isn't testable in a weekend | Strong — offline HM demo + **mentor sign-off** + **MIMIC-4 concordance study** + **Namibia GMO design partners** |
...
That hits category 4 (a real, testable MVP), proves category 2 (offline = feasible), and the live WiFi-off moment is the category 6 mic-drop. Beyond the demo, validation rests on three pillars: **mentor neurosurgeon sign-off** on the EDH bundle (this weekend), a **MIMIC-4 concordance study** (guidance vs. real TBI clinical trajectories — concordance, not an outcome RCT), and **Namibia GMO design partners** (real users from day one). Detail in [07-kyro-product-and-pitch.md](07-kyro-product-and-pitch.md).
