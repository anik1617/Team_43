...
- **L2 — Knowledge.** A small, curated, source-cited **GraphRAG** graph, built on the cloud build plane (cloud LLM API + light compute; no supercomputer) and shipped as a signed SQLite bundle (`edh-core-v{N}.kyro`). Citations and question-ordering come from the graph with no LLM call.
