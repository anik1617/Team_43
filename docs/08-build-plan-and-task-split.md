...
1. **`embedder_id` + `embedder_dim`** — **BGE-M3, 1024-d, BYTE-IDENTICAL model on both planes.** *Correction (approach A, verified against current GraphRAG docs):* Microsoft GraphRAG has **no native BGE-M3 provider** (only `openai` / `azure` + an `api_base`), so we do **not** rely on it for the shipped vectors. GraphRAG does **graph extraction only**; the **C5 compiler (`build_bundle.py`) re-embeds chunk/node text with BGE-M3 itself** via `kyro_bundle/embedders.py`. The parity that actually matters is therefore **our `BgeM3Embedder` ↔ llama.rn's BGE-M3 on the phone**, NOT GraphRAG. *Still the #1 project-killing gotcha — just relocated to the compiler↔device seam. Do not try to "override embeddings to BGE-M3 in settings.yaml"; that path doesn't exist.*
...
| C3 | **Graph build (extraction only)** | **Microsoft GraphRAG** + a **cloud LLM API** (Claude/GPT). GraphRAG's own **embeddings are DISCARDED** (no native BGE-M3 provider); the **C5 compiler re-embeds with BGE-M3** (dim 1024) so shipped vectors share one code path with the device. | a ~160-node corpus is a **small** build — **no supercomputer needed**. **160-node-class graph is proven sufficient** — do not over-build. Setup: `cloud/graphrag/README.md`. |
