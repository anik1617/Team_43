...
| VF-PED | N98 pediatric scope | age<15 → out-of-scope abstain (no adult thresholds applied). | Confirm pediatric is intentionally OUT of v1 MVP and abstain-and-transfer (not an encoded peds pathway) is the safe default; confirm the 70+2×age labeled SBP principle. | 2 |
...
6. **B3/B6 safe-side divergences** — confirm peds→abstain+transfer and anticoag→transfer (rather than local observe) are the intended conservative defaults.
